...
const Product = ({ product, compactOnMobile = false }) => {
  ...
  return (
    <article className={`group flex h-full min-w-0 max-w-full flex-col overflow-hidden border border-[#dce4ef] bg-white shadow-[0_20px_50px_rgba(11,31,58,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_70px_rgba(11,31,58,0.14)] ${compactOnMobile ? 'rounded-[18px] sm:rounded-[28px]' : 'rounded-[28px]'}`}>
      <Link
      ...
      >
        <div className={`relative overflow-hidden bg-[#ecf0f7] ${compactOnMobile ? 'aspect-[4/3] sm:aspect-auto sm:h-72' : 'h-72'}`}>
          <img
      ...
      <div className={`flex min-w-0 flex-1 flex-col ${compactOnMobile ? 'p-3 sm:p-5' : 'p-5'}`}>
        ...
        <Link
          to={`/product/${product._id}`}
          className={compactOnMobile ? 'mt-2 block min-w-0 sm:mt-3' : 'mt-3 block min-w-0'}
          onMouseEnter={preloadProductPage}
          onFocus={preloadProductPage}
          onTouchStart={preloadProductPage}
        >
          <h3 className={`line-clamp-2 break-words font-serif font-bold text-[#081729] transition-colors duration-200 group-hover:text-[#a07c16] ${compactOnMobile ? 'text-base leading-5 sm:text-2xl sm:leading-tight' : 'text-2xl leading-tight'}`}>
            {product.name}
          </h3>
        </Link>
        ...
        <div className={`mt-auto flex flex-col border-t border-[#e6ebf4] ${compactOnMobile ? 'gap-3 pt-3 sm:gap-4 sm:pt-4' : 'gap-4 pt-4'}`}>
          <div>
            ...
            <p className={`break-all font-serif font-bold text-[#081729] ${compactOnMobile ? 'text-[clamp(1rem,5vw,1.25rem)] leading-tight tracking-[-0.02em] sm:text-3xl sm:leading-tight sm:tracking-normal' : 'text-3xl leading-tight'}`}>
              {formatPrice(product.price)}
            </p>
          </div>

          <div className="grid w-full min-w-0 grid-cols-2 items-center gap-2 max-[359px]:grid-cols-1">
            <Link
              to={`/product/${product._id}`}
              className={`inline-flex min-w-0 items-center justify-center rounded-full border border-[#ccd8e8] font-bold uppercase text-[#2c4a73] transition-colors duration-200 hover:border-[#2c4a73] hover:bg-[#eef2f8] ${compactOnMobile ? 'px-1.5 py-2 text-[9px] tracking-[0.06em] sm:px-4 sm:text-xs sm:tracking-[0.16em]' : 'px-4 py-2 text-xs tracking-[0.16em]'}`}
            >
              View <ArrowRight size={13} className="ml-1 shrink-0 sm:ml-2" />
            </Link>
            ...
            {product.countInStock === 0 ? (
              <button
                type="button"
                disabled
                className={`inline-flex min-w-0 items-center justify-center rounded-full bg-gray-200 font-bold uppercase text-gray-500 ${compactOnMobile ? 'px-1.5 py-2 text-[9px] tracking-[0.06em] sm:px-4 sm:text-xs sm:tracking-[0.16em]' : 'px-4 py-2 text-xs tracking-[0.16em]'}`}
              >
                <Lock size={13} className="mr-1 shrink-0 sm:mr-2" /> Sold Out
              </button>
            ) : (
              <button
                type="button"
                onClick={handleAddToCart}
                className={`inline-flex min-w-0 items-center justify-center rounded-full bg-[#22406b] font-bold uppercase text-white transition-colors duration-200 hover:bg-[#081729] ${compactOnMobile ? 'px-1.5 py-2 text-[9px] tracking-[0.06em] sm:px-4 sm:text-xs sm:tracking-[0.16em]' : 'px-4 py-2 text-xs tracking-[0.16em]'}`}
              >
                <ShoppingCart size={13} className="mr-1 shrink-0 sm:mr-2" /> {product.allowCustomQuantity ? 'Select' : 'Add'}
              </button>
            )}
          </div>
        </div>
